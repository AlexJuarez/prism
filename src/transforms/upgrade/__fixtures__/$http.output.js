...
function cloLtiService(
  ...
  function getEpubURL(asset) {
    let shortName, chapterId, assetIdParts;
    ...
    return $q
      ...
      .then(() => {
        if (selectedCourse.themeUuid) {
          return courseService.theme
            .getTheme(selectedCourse.themeUuid)
            .then(theme => {
              courseTheme = theme;
            });
        }
      ...
      .then(epubObj => {
        ...
        let ltiPayload = <any>{
          ...
          tool_consumer_instance_description:
            session.User.organizationDescription,
          ...
          lis_person_sourcedid:
            session.User.organizationDomain + ':' + session.User.id,
          ...
          // Launch leveled text
          custom_display_levels:
            'on_level,below_level,approaching_level,above_level,english_language_development',
        ...
        if (courseTheme && !courseTheme.isDefaultTheme) {
          ltiPayload = {
            ...
            launch_presentation_css_url:
              courseTheme.url + sessionProvider.getTokenAsParam()
          };
        }
  ...
  function getPlayerUrlFromLti(parameters, authUrl) {
    ...
    return courseService
      .signCloLti(parameters)
      .then(function(response) {
        return this.http.post(
          envSettings.clo.playerApi + '/lti',
          response,
          configuration
        );
      })
