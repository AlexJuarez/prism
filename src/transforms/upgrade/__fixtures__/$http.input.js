cloLtiService.$inject = ['$http', '$q', 'appSettings', 'courseService', 'envSettings', 'sessionProvider'];

function cloLtiService($http, $q, appSettings, courseService, envSettings, sessionProvider) {
    let epubcfi;
    ...
    function getPlayerUrlFromLti(parameters, authUrl) {
        ...
        return courseService.signCloLti(parameters)
            .then(function(response) {
                return $http.post(envSettings.clo.playerApi + '/lti', response, configuration);
            })
            .then(function (response) {
                const epubUrls = {
